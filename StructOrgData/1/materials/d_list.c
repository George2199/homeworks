...
void insert (struct element * cur, DataType x) /*вставка после текущего (не последнего!)*/
{
       ...
       temp->prev = cur;
       cur->next = temp;
       temp->next->prev = temp; /*обновляем prev следующего элемента добавленным после вставленного*/
}

void del_list (struct List list) /*удаление списка*/
{
     struct element * temp;
     while (list.head) /*если в списке есть элементы*/
     {
           temp = list.head;
...
struct List del_head (struct List list) /*удаление первого элемента*/
{
     ...
     if (list.head) /*если в списке есть элементы*/
     {
           temp = list.head;
           list.head = list.head->next; /*переставляем указатель на следующий элемент*/
           if (list.head == NULL) /*если удалённый элемент был единственным*/
               list.tail = NULL;
           else
               list.head->prev = NULL; /*обнуляем указатель на удалённый элемент*/
           free (temp); /*первый удаляем*/
     }
     return list;
}
...
struct List del_tail (struct List list) /*удаление последнего элемента*/
{
     ...
     if (list.tail) /*если в списке есть элементы*/
     {
           ...
           if (list.tail == NULL) /*если удалённый элемент был единственным*/
                list.head = NULL;
           else
                list.tail->next = NULL; /*обнуляем указатель на удалённый элемент*/
           free (temp); /*последний удаляем*/
     }
     return list;
}

void del_element (struct element * cur) /*удаление элемента (любой(!) не первого и не последнего)*/
{
       cur->next->prev = cur->prev; /*обновляем prev следующего элемента указателем через предшествующего удаляемому*/
       cur->prev->next = cur->next; /*обновляем next предыдущего элемента указателем через следующий за не последним*/
       free (cur);
}
...
{
     while (list.tail) /*если в списке есть элементы*/
     {
